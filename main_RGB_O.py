#This RGB-only model is based on SimpleNet architectute 
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        rgbOmodel_list = methods["get_rgbOmodel"](imagesize, device)

        models_dir = os.path.join(run_save_path, "models")
        os.makedirs(models_dir, exist_ok=True)
        for i, RgbOmodel in enumerate(rgbOmodel_list):
            # torch.cuda.empty_cache()
            if RgbOmodel.backbone.seed is not None:
                utils.fix_seeds(RgbOmodel.backbone.seed, device)
            LOGGER.info(
                "Training models ({}/{})".format(i + 1, len(rgbOmodel_list))
            )
            # torch.cuda.empty_cache()

            RgbOmodel.set_model_dir(os.path.join(models_dir, f"{i}"), dataset_name)
            if not test:
                i_auroc, p_auroc, pro_auroc = RgbOmodel.train(dataloaders["training"], dataloaders["testing"])
            else:
...
def net(
    ...
    def get_rgbOmodel(input_shape, device):
        rgbOmodels = []
        for backbone_name, layers_to_extract_from in zip(
            ...
            rgbOmodel_inst = rgbOmodel.RgbOmodel(device)
            rgbOmodel_inst.load(
                backbone=backbone,
            ...
            rgbOmodels.append(rgbOmodel_inst)
        return rgbOmodels

    return ("get_rgbOmodel", get_rgbOmodel)
